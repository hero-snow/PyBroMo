...
def find_git():
    ...
    for git_path_i in GIT_PATHS:
        if git_path_i is not None and os.path.isfile(git_path_i) and os.access(git_path_i, os.X_OK):
            git_path = git_path_i
            break
    return git_path
...
def git_path_valid(git_path=None) -> bool | None:
...
def get_git_version(git_path=None):
    ...
    if git_path is None:
        git_path = GIT_PATH
    return check_output([git_path, "--version"]).split()[2]
...
def get_status(git_path=None):
    ...
    if git_path is None:
        git_path = GIT_PATH
    return check_output([git_path, "status", "--porcelain"])
...
def get_last_commit_line(git_path=None):
    ...
    if git_path is None:
        git_path = GIT_PATH
    output = check_output([git_path, "log", "--pretty=format:'%ad %h %s'", "--date=short", "-n1"])
...
def print_summary(string="Repository", git_path=None) -> None:
    ...
    if not git_path_valid():
        print(f"\n{string} revision unknown (git not found).")
    else:
