try:
    from PyQt2.QtGui import QSlider, QToolBar
    from PyQt4 import QtCore, QtGui
except ImportError:
    from PyQt5 import QtCore
    from PyQt5.QtWidgets import QSlider, QToolBar
...
class ScrollPlotter:
    ...
    def __init__(self, time_size, duration, t_step, decimate) -> None:
    ...
    def set_slider(self, parent) -> None:
        self.slider = QSlider(QtCore.Qt.Horizontal, parent=parent)
        self.slider.setTickPosition(QSlider.TicksAbove)
        self.slider.setTickInterval((self.smax - self.smin) / 10.0 * self.scale)
    ...
    def slider_changed(self, pos) -> None:
        pos /= self.scale
    ...
    def update(self, slice_=None) -> None:
        pass


class EmissionPlotter(ScrollPlotter):
    def __init__(self, S, particles=None, color_pop=True, duration=0.01, decimate=100) -> None:
    ...
    def init_plot(self) -> None:
        ...
        self.title_suffix = f"Total: {self.S.t_step * self.S.n_samples:.1f} s, Visualized: {self.duration * 1e3:.2f} ms"
    ...
    def update(self, slice_=None) -> None:
        ...
        for ip, l_em in zip(self.particles, self.lines_em, strict=False):
            l_em.set_ydata(emission[ip])
            self.ax.draw_artist(l_em)
        time = slice_.start * self.S.t_step
        self.title.set_text(f"t = {time:5.3f}s {self.title_suffix}")
...
class TrackEmPlotterR(ScrollPlotter):
    ...
    def plot_psf(self) -> None:
        psf = self.S.psf.hdata
        cmap = cm.YlGnBu
        cmap.set_under(alpha=0)
        kwargs = {"interpolation": "nearest", "cmap": cmap, "vmin": 1e-1, "zorder": 1}
    ...
    def init_plot(self) -> None:
        ...
        for ip in self.particles:
            ...
            em_kw = {"alpha": 0.8, "ls": "-", "color": color}
        ...
        if len(self.particles) <= 20:
            AX[0].legend(bbox_to_anchor=(1.01, 1), loc=2, borderaxespad=0.0)
    ...
    def update(self, slice_=None) -> None:
        ...
        self.fig.canvas.restore_region(self.background)
        for ip, l_rz, l_em in zip(self.particles, self.lines_rz, self.lines_em, strict=False):
        ...
        time = slice_.start * self.S.t_step
        self.title.set_text(f"t = {time:5.1f}s {self.title_suffix}")
        self.fig.draw_artist(self.title)
        self.fig.canvas.blit(self.fig.bbox)
...
class TrackEmPlotter(ScrollPlotter):
    ...
    def init_plot(self) -> None:
        fig, AX = self.fig, self.AX
        plt.subplots_adjust(left=0.06, right=0.96, top=0.95, bottom=0.09, wspace=0.05)
        self.title_suffix = f"Total: {self.S.t_step * self.S.n_samples:.1f} s, Visualized: {self.duration * 1e3:.2f} ms"
    ...
    def update(self, slice_=None) -> None:
        ...
        self.fig.canvas.restore_region(self.background)
        for ip, l_xy, l_zy, l_em in zip(self.particles, self.lines_xy, self.lines_zy, self.lines_em, strict=False):
