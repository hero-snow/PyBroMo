...
class GaussianPSF:
    ...
    def __init__(self, xc=0, yc=0, zc=0, sx=0.2e-6, sy=0.2e-6, sz=0.8e-6, psf_pytables=None) -> None:
    ...
    def __repr__(self) -> str:
        return f"GaussianPSF( { {'mean': self.rc, 'sigma': self.s} } )"
    ...
    def to_hdf5(self, file_handle, parent_node="/"):
        ...
        tarray = file_handle.create_array(
            parent_node, name="gauss_psf_params", obj=data, title="Gaussian PSF mean and std. dev."
        )
...
class NumericPSF:
    def __init__(
        ...
        xi, zi, hdata, zm = convert_PSFLab_xz(self.psflab_psf_raw, x_step=x_step, z_step=z_step, normalize=True)
    ...
    def to_hdf5(self, file_handle, parent_node="/"):
        ...
        """
        tarray = file_handle.create_array(
            parent_node, name=self.fname, obj=self.psflab_psf_raw, title="PSF x-z slice (PSFLab array)"
        )
    ...
    def hash(self):
        """Return an hash string computed on the PSF data."""
        hash_list = []
        for _key, value in sorted(self.__dict__.items()):
...
def load_PSFLab_file(fname):
    ...
    msg = f"Can't find PSF file '{fname}'"
    raise OSError(msg)
