...
def sim_timestamps_em_list(S, max_rate=1, bg_rate=0, rs=None, seed=None) -> None:
    ...
    for i_start, i_end in iter_chunk_index(S.n_samples, S.emission.chunkshape[1]):
        counts_chunk = sim_timetrace(S.emission[:, i_start:i_end], max_rate, S.t_step)
        counts_bg_chunk = rs.poisson(bg_rate * S.t_step, size=counts_chunk.shape[1]).astype("uint8")
        ...
        for p_i, counts_chunk_p_i in enumerate(counts_chunk.copy()):
            ...
            for frac, v in zip(fractions, range(2, max_counts + 1), strict=False):
        ...
        par_index_chunk_p.append(np.full(time_chunk_bg.size, p_i + 1, dtype="u1"))
...
def sim_timestamps_em_list1(S, max_rate=1, bg_rate=0, rs=None, seed=None) -> None:
    """Compute timestamps and particles and store results in a list.
    Each element contains timestamps from one chunk of emission.
    ...
    # Load emission in chunks, and save only the final timestamps
    for i_start, i_end in iter_chunk_index(S.n_samples, S.emission.chunkshape[1]):
        counts_chunk = sim_timetrace_bg(S.emission[:, i_start:i_end], max_rate, bg_rate, S.t_step, rs=rs)
        index = np.arange(0, counts_chunk.shape[1])
...
def sim_timestamps_em_list2(S, max_rate=1, bg_rate=0, rs=None, seed=None) -> None:
    """Compute timestamps and particles and store results in a list.
    Each element contains timestamps from one chunk of emission.
    ...
    # Load emission in chunks, and save only the final timestamps
    for i_start, i_end in iter_chunk_index(S.n_samples, S.emission.chunkshape[1]):
        counts_chunk = sim_timetrace_bg2(S.emission[:, i_start:i_end], max_rate, bg_rate, S.t_step, rs=rs)
        index = np.arange(0, counts_chunk.shape[1])
