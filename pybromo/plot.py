...
def plot_tracks(S) -> None:
    _fig, AX = plt.subplots(2, 1, figsize=(6, 9), sharex=True)
    plt.subplots_adjust(left=0.12, right=0.95, top=0.95, bottom=0.05, hspace=0.05)
    plt.suptitle("%.1f ms diffusion" % (S.t_step * S.n_samples * 1e3))

    for ip in range(S.np):
        x, y, z = S.pos[ip]
        x0, y0, z0 = S.particles[ip].r0
        plot_kwargs = {"ls": "", "marker": "o", "mew": 0, "ms": 1, "alpha": 0.2}
    ...
    sig = S.psf.s if S.psf.name == "gauss" else array([0.1, 0.1, 0.3]) * 1e-06
    ## Draw an outline of the PSF
    a = arange(360) / 360.0 * 2 * np.pi
...
def plot_emission(S, dec=1, scroll_gui=False, multi=False, ms=False):
    fig = plt.figure()
    plt.title("%d Particles, %.1f s diffusion, %d pM" % (S.np, S.t_step * S.n_samples, S.concentration() * 1e12))
    plt.xlabel("Time (s)")
    plt.ylabel("Emission rate [A.U.]")
    plt.grid(1)
    if ms:
        plt.xlabel("Time (ms)")
    if multi:
...
def plot_timetrace(S, rebin=2e3, scroll_gui=False):
    fig = plt.figure()
    plt.title(
        "%d Particles, %.1f s diffusion, %d pM, bin=%.1fms"
        % (S.np, S.t_step * S.n_samples, S.concentration() * 1e12, S.t_step * rebin * 1e3)
    )
    plt.xlabel("Time (s)")
    plt.ylabel("Emission rate [A.U.]")
    plt.grid(1)
