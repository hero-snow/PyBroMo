...
class ParticlesSimulation(object):
    ...
    @property
    def sigma_1d(self):
        return np.sqrt(2 * self.diffusion_coeff * self.t_step)
    ...
    def _sim_trajectories(self, time_size, start_pos, rs,
        ...
        sigma_1d_reshaped = self.sigma_1d[:, np.newaxis, np.newaxis]

        # Generate random displacements for all particles at once
        delta_pos = rs.normal(loc=0, scale=sigma_1d_reshaped,
                              size=(num_particles, 3, time_size))

        # Calculate trajectories for all particles
        ...
        if save_pos:
            if radial:
                pos_save = np.stack((Ro, Z), axis=1)
            else:
                pos_save = pos
            POS = [p[np.newaxis, :, :] for p in pos_save]
