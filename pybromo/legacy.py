...
def parallel_gen_timestamps(dview, max_em_rate, bg_rate):
    ...
    """
    dview.execute(
        "S.sim_timestamps_em_store(max_rate=%d, bg_rate=%d, seed=S.EID, overwrite=True)" % (max_em_rate, bg_rate)
    )
    ...
    times_all, times_par_all = merge_ph_times(Times, Times_par, time_block=t_max)
...
def merge_ph_times(times_list, times_par_list, time_block):
    ...
    i1 = 0
    for i2, ts, ts_par, offset in zip(cum_sizes, times_list, times_par_list, offsets, strict=False):
...
def merge_DA_ph_times(ph_times_d, ph_times_a):
    ...
    a_em = np.hstack([np.zeros(ph_times_d.size, dtype=bool), np.ones(ph_times_a.size, dtype=bool)])
...
def merge_particle_emission(SS):
    ...
    s = SS[0]
    S = ParticlesSimulation(t_step=s.t_step, t_max=s.t_max, particles=P, box=s.box, psf=s.psf)
